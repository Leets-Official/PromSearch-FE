...
> 기획 결정 확정됨(2026-07-21). `## 확정된 사항` 참고.
...
- `비로그인` : 레시피 블러 + "로그인하고 프롬프트 보기"
- `로그인·무료` : 전체 열람 + "복사하기"
- `로그인·프리미엄` : 일부 노출 후 블러 + "포인트로 전문 보기"
...
| 상황                     | 레시피 표현           | CTA                                      | 피그마              |
| ------------------------ | --------------------- | ---------------------------------------- | ------------------- |
| 비로그인                 | 전체 블러             | `로그인하고 프롬프트 보기` → 로그인 유도 | 409:8162            |
| 로그인·무료 / 잠금해제됨 | 전체 노출             | `복사하기`(복사 이벤트)                  | 409:8197 / 551:4579 |
| 로그인·프리미엄(미구매)  | 앞부분만 노출 후 블러 | `포인트로 전문 보기` → 포인트 차감 흐름  | 409:8235            |
...
- **추천(좋아요) 토글**: 카운트 표시 + 토글(목 POST + 낙관적 갱신).
...
POST /api/prompts/:id/like      → 200 { liked, likeCount } (토글)
```
...
- 추천 토글 목: 현재 liked/카운트를 토글해 반환(낙관적 갱신 롤백 테스트용).
...
| ---------------------- | ------------------------- | ------------------------------------- |
| 비로그인               | `""`(미전송)              | **더미 플레이스홀더** 전체 블러 + CTA |
| 프리미엄(미구매)       | 앞 `previewLength` 티저만 | 티저 노출 + 그 아래 더미 블러 + CTA   |
...
- **추천 토글** → `useLikePrompt` 뮤테이션(목 `POST /:id/like`) + 낙관적 카운트 갱신, 실패 시 롤백.
...
6. **추천(좋아요)** — **토글 동작 포함**(목 `POST /:id/like` + 낙관적 갱신).
