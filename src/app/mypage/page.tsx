      ...
      <div className="flex flex-col gap-4 lg:flex-row">
        ...
        <Card className="relative flex-1 flex-col items-center gap-4 text-center sm:flex-row sm:gap-5 sm:text-left">
          ...
          <div className="flex flex-col items-center gap-2 sm:items-start">
            <span className="text-heading-2 text-text-primary">{profile.nickname}</span>
            {/* 모바일: 이메일 */}
            <span className="text-body-3 text-text-secondary sm:hidden">{profile.email}</span>
            {/* sm+: 관심 태그 */}
            <div className="hidden flex-wrap gap-1.5 sm:flex">
              {[...profile.interestJobTags, ...profile.interestTaskTags].map((tag) => (
